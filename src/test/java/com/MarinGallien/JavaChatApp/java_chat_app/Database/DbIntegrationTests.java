...
@DataJpaTest
@ActiveProfiles("test")
@Import(WebSocketDbService.class)
class DbIntegrationTests {
    ...
    @Autowired
    private WebSocketDbService WebSocketDbService;
    ...
    void setUp() {
        ...
        String chatId = "PRIVATE_" + user1.getUserId() + "_" + user2.getUserId();
        chat1 = new Chat(chatId, ChatType.SINGLE);
    ...
    // ==========================================================================
    // TESTING BUSINESS LOGIC IN WebSocketDbService
    // ==========================================================================
    ...
    void saveMessage_ValidMessage_SavesSuccessfully() {
        ...
        // When
        Message savedMessage = WebSocketDbService.saveMessage(wsMessage);
    ...
    void saveStatus_ValidUser_UpdatesStatus() {
        ...
        // When
        boolean result = WebSocketDbService.saveStatus(user1.getUserId(), OnlineStatus.ONLINE);
    ...
    void saveStatus_NonExistentUser_ReturnsFalse() {
        // When
        boolean result = WebSocketDbService.saveStatus("fake-user-id", OnlineStatus.ONLINE);
    ...
    void getContacts_ValidUser_ReturnsContacts() {
        // When
        List<String> contacts = WebSocketDbService.getContacts(user1.getUserId());
    ...
    void getContacts_NonExistentUser_ReturnsEmptyList() {
        // When
        List<String> contacts = WebSocketDbService.getContacts("fake-user-id");
    ...
    void getAllChatParticipantMappings_ReturnsCorrectMappings() {
        // When
        List<Object[]> mappings = WebSocketDbService.getAllChatParticipantMappings();
    ...
    void userExists_ExistingUser_ReturnsTrue() {
        // When
        boolean exists = WebSocketDbService.userExists(user1.getUserId());
    ...
    void userExists_NonExistentUser_ReturnsFalse() {
        // When
        boolean exists = WebSocketDbService.userExists("fake-user-id");
    ...
    void chatExists_ExistingChat_ReturnsTrue() {
        // When
        boolean exists = WebSocketDbService.chatExists(chat1.getChatId());
    ...
    void chatExists_NonExistentChat_ReturnsFalse() {
        // When
        boolean exists = WebSocketDbService.chatExists("fake-chat-id");
    ...
    void isUserInChat_UserInChat_ReturnsTrue() {
        // When
        boolean inChat = WebSocketDbService.isUserInChat(user1.getUserId(), chat1.getChatId());
    ...
    void isUserInChat_UserNotInChat_ReturnsFalse() {
        ...
        boolean inChat = WebSocketDbService.isUserInChat(user3.getUserId(), chat1.getChatId());
